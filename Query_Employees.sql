...
-- (Some employees are listed in multiple departments and will have a row for each department)
...
FROM employees e
LEFT JOIN dept_emp de ON e.emp_no = de.emp_no
WHERE de.dept_no = (SELECT dept_no FROM departments WHERE dept_name LIKE 'Sales%');
